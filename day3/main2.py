        ...
        if char == "*":
            ...
            for obj in collectedInfo:
                ...
                if rows[cRow][cIdx + 1].isdigit() and rows[cRow][cIdx - 1].isdigit():
                    rightSide = ""
                    leftSide = ""
                    for j in range(cIdx, len(rows[cRow])):
                    
                        if rows[cRow][j] == "." or isSymbol(rows[cRow][j]):
                            break
                        
                        rightSide += rows[cRow][j]
                    
                    for j in range(0, cIdx):
                        if rows[cRow][j] == "." or isSymbol(rows[cRow][j]):
                            leftSide = ""
                        else:
                            leftSide += rows[cRow][j]
                    ...
                    for j in range(cIdx, len(rows[cRow])):
                        ...
                        digitCollect += rows[cRow][j]
                
                elif rows[cRow][cIdx - 1].isdigit():
                    for j in range(0, cIdx + 1):
                        if rows[cRow][j] == "." or isSymbol(rows[cRow][j]):
                            digitCollect = ""
                        else:
                            digitCollect += rows[cRow][j]
            ...
            if len(numbersToMul) >= 2:
                result += numpy.prod(numbersToMul)
